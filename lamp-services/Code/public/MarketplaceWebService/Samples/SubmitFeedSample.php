...
$feed = call();
//echo $feed; 
   $feed = str_replace('<?xml version="1.0"?>', "<?xml version='1.0'?>", $feed);
   $feed = str_replace('<conf>', " ", $feed);
   $feed = str_replace('</conf>', " ", $feed);
   $feed = str_replace('<root>', " ", $feed);
   $feed = str_replace('</root>', " ", $feed);
...
return file_get_contents('C:\xampp\htdocs\MarketplaceWebService\sample123.xml');
}
