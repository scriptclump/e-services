...
<table width="100%" border="0" cellspacing="5" cellpadding="5">
  <tr>
    <td width="50%" align="left" valign="middle">
        ...
        @endif
        <strong style="float:left; line-height:32px; ">{{$leInfo->business_legal_name}}</strong></td>
...
<table width="100%" style="font-size:11px; border-collapse:collapse; border-right:1px solid #000;" cellspacing="0" cellpadding="2" border="1">
  ...
  <tr>
    <td valign="top" style="padding:5px;"><strong>Name:</strong> {{$orderDetails->firstname}} {{$orderDetails->lastname}} <br>
      ...
      @endif </td>
    <td valign="top" style="padding:5px;" >@if(is_object($shipping)) <strong>{{$orderDetails->shop_name}}</strong><br>
      ...
      @if(!empty($shipping->mobile)) Mobile: {{$shipping->mobile}} @endif
      @endif 
      @if(!empty($shipping->locality))<strong> Locality: </strong>{{$shipping->locality}} <br>@endif
    ...
    <td valign="top" style="padding:5px;"><strong>Order ID:</strong> {{$orderDetails->order_code}}<br>
      ...
      @if(!empty($lewhInfo->le_wh_code))<br>
      <strong>DC No:</strong> {{$lewhInfo->le_wh_code}}<br>
      @endif @if(isset($lewhInfo->lp_wh_name))<strong>DC Name:</strong> {{(isset($lewhInfo->lp_wh_name) ? $lewhInfo->lp_wh_name : '')}}<br>
      @endif
      <strong>Jurisdiction Only:</strong> Hyderabad
      @if(isset($userInfo->firstname) && isset($userInfo->lastname)) <br>
      <strong>Created By</strong>: {{$userInfo->firstname}} {{$userInfo->lastname}} (M: {{isset($userInfo->mobile_no) ? $userInfo->mobile_no : ''}})
      @endif </td>
  </tr>
</table>


<table id="sample" width="100%" style="font-size:12px !important; border-collapse:collapse; border-right:1px solid #000; margin-top:10px;" cellspacing="0" cellpadding="2" border="1">
  <thead>
