...
$invoicedQty = isset($invoiceArr[$product->product_id]) ? $invoiceArr[$product->product_id] : 0;
...
?>
  ...
  <td align="center"><p>{{$sno}}</p></td>
    ...
    <td align="right">Ordered: {{(int)$orderQty}}
    @if($invoicedQty) <br> Invoice: {{(int)$invoicedQty}} @endif
    @if($cancelQty) <br> Cancel: {{(int)$cancelQty}} @endif
    @if($returnedQty) <br> Return: {{(int)$returnedQty}} @endif
    </td>
