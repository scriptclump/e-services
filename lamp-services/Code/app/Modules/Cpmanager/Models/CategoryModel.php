<?php
  ...
  class CategoryModel extends \Eloquent {
    ...
    */
      ...
      public function getCategories($segment_id,$le_wh_id,$sync_date){
      //DB::enableQueryLog();
     /* $categories =  DB::table('mp_categories as cat')
      ...
      $categories = DB::select("CALL getCPCategories($le_wh_id,$segment_id,'".$sync_date."',0,0) ");
...
if(isset($sort_column)){
    ...
    */
    public function checkCustomerToken($customer_token){
     ...
     // db::enableQueryLog();
      $query=DB::selectFromWriteConnection(DB::raw('select verifyToken("'.$customer_token.'") as count'));
      return isset($query[0]->count)?$query[0]->count:'';   
...
//$offset = 20;
    ...
    */ 
    public function checkSegmentId($segment_id){
      $query = DB::table("master_lookup as ml")
      ->select(DB::raw("count(ml.value) as count")) 
      ->where("ml.value","=",$segment_id)   
      ->get()->all();
    ...
    public function getUserId($customer_token){
      ...
      $query=$account->getDataFromToken(2,$customer_token,['user_id','firstname','lastname','legal_entity_id']);
...
if(isset($sort_column)){
    ...
    */
    ...
    public function getCustomPackDataById($pack_id){
      $cusPackData  = DB::table("custom_packs_config")
                      ->select("*",DB::raw("(SELECT le_wh_id FROM custom_packs WHERE cp_pack_id=1 ) AS le_wh_id"))
                      ->where("cp_pack_id",$pack_id)
                      ->get()->all();
      return $cusPackData;
    }
