...
<div class="row">
    <div class="col-md-12">
        <div class="timeline">
            @if(isset($history)) @foreach($history as $historyVal )
                <?php
                    $url = public_path();
                    ...
                    <div class="timeline-body">
                        ...
                        <div class="row">
                            <div class="col-md-2 col-sm-4">
                                <p>{{ucwords($historyVal['firstname']).' '.ucwords($historyVal['lastname'])}}
                                    <span>{{$historyVal['name']}}</span></p>
                            </div>
                            <div class="col-md-2 col-sm-2">
                                <?php echo date('d/m/Y h:i A', strtotime($historyVal['created_at'])); ?>
                            </div>
                            <div class="col-md-3 col-sm-2 push_right">{{$historyVal['master_lookup_name']}}</div>
                            <div class="col-md-3 col-sm-2 push_right">{!! $historyVal['awf_comment'] !!}</div>
                        </div>
                    </div>
                </div>
                @endforeach @endif
