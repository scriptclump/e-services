      ...
      @if($cat1->category_id == $parent_cat->parent_id)
          ...
          @endif
           @foreach($product_class as $product_class_cat)
            @if($product_class_cat->is_product_class==1 && $parent_cat->category_id ==$product_class_cat->parent_id )<option value="{{ $product_class_cat->category_id}}" class="prod_class"  style="font-size:14px; font-weight:bold; color:#0174DF;">{{ $product_class_cat->cat_name}}</option> 
             @endif
            @endforeach
