...
@section('content')
<?php View::share('title', 'Ebutor - ELP Trends'); ?>
	<span id="success_message"></span>
	<div class="portlet light tasks-widget">
		<div class="portlet-title">
			<div class="caption">{{ trans('headings.LP') }} Trends</div>
			<div class="actions">
		...
		<div  class="portlet-body" id="default-report-component" ng-app="KPIReports"  ng-controller="KpiSubController">
			<div id="filters" style="display: none;">
				...
				{{ Form::open(array('url' => '/kpi/elpdownload', 'id' => 'downloadexcel'))}}
				<div class="col-md-3">
				  	<label>DC</label>
					<select ng-model="kpi_dc"  id="kpi_dc" name="kpi_dc" class="form-control" ng-change="getHubSortData(kpi_dc)">
				...
				<div class="col-md-3">
					...
					<select ng-model="date_range" id="date_range" name="date_range" class="form-control" ng-change="change_date()">
...
var app = angular.module("KPIReports", ['ui.grid','ui.grid.pagination', 'ui.grid.exporter','ui.grid.pinning','ui.grid.resizeColumns', 'ui.grid.moveColumns'],function($interpolateProvider){
...
app.controller('KpiSubController', function ($scope,$http,$rootScope,$filter,$timeout,uiGridConstants) {
		...
		$scope.loadAnalysis = function(){
			...
			var req = $http.post('purchasedata',$scope.post_data);
			$("#default-report-component").addClass("disabledbutton");
			var salesReturnHeader = [];
			...
			$("#return_grid_data").html("No Data found!");
				$scope.salesReturnData = successCallback;
				if($scope.salesReturnData.status == "true"){
					$("#return_grid").show();
        			...
        			$scope.damageReportData = $scope.salesReturnData.data;
        			angular.forEach($scope.damageReportData[0],function(val,key){
        				...
        				salesReturnHeader.push({key:key,
        			...
        			console.log(salesReturnHeader);

        			$('#resultGrid').igGrid({
		                 dataSource: $scope.damageReportData,
		                 dataSourceType: "json",
		                 columns: salesReturnHeader,
...
@if($elpAccess == 0)
<script type="text/javascript">
	$("#return_grid_data").html("No Access!");
</script>
@endif
