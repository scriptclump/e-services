...
use App\Modules\KPIReports\Models\nctAnalysisModel;
...
class nctAnalysisController extends BaseController{
	
	function __construct()
	{
		$this->commonController=new commonController();
		$this->_KpiModel=new nctAnalysisModel();
	}
	

	public function nctAnalysis(){

		$breadCrumbs = array('HOME' => url('/'),'Reports' => '#', 'NCT Trends' => '#');
		...
		$DC_list=array();
     	...
     	return view('KPIReports::nctAnalysis',['DC_list' => $dcData,'HUB_list'=>$hubData,"Beat_List"=>$beatData,"outlet_List"=>$outletdata,"so_List"=>$SOData,"dc_hub_data"=>$dc_hubData]);

	}

	public function getnctData(Request $request){
		...
		$data = $this->_KpiModel->getnctDetailsDb($dc,$hub_id,$beat_id,$outlet_id,$so_id,$start_date,$end_date);
		if(count($data) <= 0){
			$data = 'No Data Found !';
			$status = "false";
		}else{
