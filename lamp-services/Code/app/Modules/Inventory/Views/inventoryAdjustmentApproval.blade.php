...
$('#approval-workflow-form').validate({
    ...
    submitHandler: function (form) {
        ...
        $('#approval_submit').attr("disabled", "disabled");
          $.ajax({
            type: "POST",
            url: "/inventory/invadjbulkapprovalworkflowdetails?_token=" + token,
            data: $("#approval-workflow-form").serialize(),
            // processData: false,
            // contentType: false,
            // dataType: "json",
            ...
            success: function (data)
            {
                ...
                else if(data ==2)
                {
                    ...
                    $.each(data, function(i, e){
                           table_data+= "<tr><td>"+data[i].product_id+"</td><td>"+data[i].product_title+"</td><td>"+data[i].upl_excess+"</td><td>"+data[i].cur_dit+"</td><td>"+data[i].up_dit_qty+"</td><td>"+data[i].cur_dnd_qty+"</td><td>"+data[i].up_dnd_qty+"</td></tr>"
                        });
