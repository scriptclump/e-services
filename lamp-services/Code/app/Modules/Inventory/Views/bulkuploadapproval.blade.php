...
<!--                                     <th>SOH Old Value</th>
                                    ...
                                    <th>DIT Old Value</th>
                                    <th>DIT Difference</th>
                                    <th>DIT New Value</th>


                                    <th>Missing Old Value</th>
                                    <th>Missing Difference</th>
                                    <th>Missing New Value</th>
                                    <th>Excess</th>
                                    <th>Reason</th>
                                    <th>Comment</th>
                            ...
                            <tbody>
                                @foreach ($bulkdetails as $value)
                                   <tr> 
                                        ...
                                        <td>{{ $value['curr_dit_qty'] }}</td>
                                        <td>{{ $value['dit_diff'] }}</td>
                                        <td>{{ $value['resulted_dit'] }}</td> 

                                        <td>{{ $value['curr_dnd_qty'] }}</td>
                                        <td>{{ $value['dnd_diff'] }}</td>
                                        <td>{{ $value['resulted_dnd'] }}</td>
                                        <td>{{ $value['excess'] }}</td> 

                                        <td>{{ $value['master_lookup_name'] }}</td>
                                        <td>{{ $value['remarks'] }}</td>
                                   </tr>
                                @endforeach
                            </tbody>
                ...
                <div class="row display_div" id="message_div">
                    ...
                    <div class="col-md-12 text-center" id="db_error">
                        Something Went Wrong,Please Try after sometime, <a href="javascript:closeWindow();">Close tab!</a>
                    </div>
                </div>
                @endif
...
$('#approval-workflow-form').validate({
    ...
    submitHandler: function (form) {
        ...
        var token = $("#take").val();
        $('#approval_submit').attr("disabled", "disabled");
          $.ajax({
            type: "POST",
            url: "/inventory/bulkapprovalworkflowdetails?_token=" + token,
            data: $("#approval-workflow-form").serialize(),
            // processData: false,
            // contentType: false,
            // dataType: "json",
            ...
            success: function (data)
            {
                ...
                else if(data ==2)
                {
                    ...
                    $.each(data, function(i, e){
                           table_data+= "<tr><td>"+data[i].product_id+"</td><td>"+data[i].product_title+"</td><td>"+data[i].inv_soh+"</td><td>"+data[i].cur_soh+"</td><td>"+data[i].dit_qty+"</td><td>"+data[i].dnd_qty+"</td><td>"+data[i].quarantine_qty+"</td></tr>"
                        });
                    $("#inv_table_id").append(table_data);
                    $("#array_success").show();
                    $("#after_submit").hide();
                    $("#after_success").hide();
                    $("#db_error").hide();
