...
/*
  Filename : AttributeController.php
  Author : Ebutor
  CreateData : 20-Aug-2016
  Desc : Controller for Attribute setts and Attribute values
 */
use View;
...
class AttributeController extends BaseController {
    ...
    public function __construct() {
        ...
        try {
            ...
            parent::Title('Manage Attribute Sets');
    ...
    public function index() {
        $breadCrumbs = array('Home' => url('/'), 'Product Templates' => url('/'), 'Attribute Sets' => url('#'));
        ...
        return View::make('Attributes::attributeHierarchicalGridDisplay')->with([]);
    }
    ...
    public function uploadexcelsheet(Request $request) {
        if (Input::hasFile('upload_doc')) {
            ...
            if ($data['header_data']       != $templateHeaders['header_data']) {
                // $result = 0;
                print_r(json_encode(array('failed'=>"Headers mis-matched in Excel sheet",'success'=>0,'update'=>0)));
                die;
            } else {
                $result = $this->_taxclass->storeTaxrules($taxdata);
            print_r(json_encode($result));
            }
        }
    }
    ...
    public function onlyStateNames(Request $req) {
        ...
        foreach ($onlystates as $onlystate) {
            if($taxClassAccess == 1){
                $onlystates[$i]['actions'] = '<code style="cursor: pointer;"><a data-type="add" id="clickState"  data-id="' . $onlystate['zone_id'] . '" data-toggle="modal" data-target="#createrule-modal"><span  style="padding-left:15px;"><i class="fa fa-plus"></i></span></a></code>';
            }
            else{
