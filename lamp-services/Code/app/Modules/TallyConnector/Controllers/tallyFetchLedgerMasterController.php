...
class tallyFetchLedgerMasterController extends BaseController{
	...
	public function fetchVoucherDetails(Request $request){
		...
		<ENVELOPE>
			...
			<BODY>
			    <DESC>
			        <STATICVARIABLES>
			        	<SVCURRENTCOMPANY>'.$currentCompanyName.'</SVCURRENTCOMPANY>
			            <SVFROMDATE TYPE="Date">09-DEC-2016</SVFROMDATE>
			            <SVTODATE TYPE="Date">09-DEC-2016</SVTODATE>
		...
		if( isset($tallyFinalResponse['BODY']['DATA']['COLLECTION']['VOUCHER']) ){
			...
			if(isset($tallyFinalResponse[0])){
				...
				foreach($tallyFinalResponse as $data){
					...
					$tallyAllVouchers[$loop_couneter]['LEDGERNAME']		=	isset($data['LEDGERNAME']) ? $data['LEDGERNAME'] : "";
				...
				$tallyAllVouchers[$loop_couneter]['LEDGERNAME']		=	isset($tallyFinalResponse['LEDGERNAME']) ? $tallyFinalResponse['LEDGERNAME'] : "";
		...
		if(count($tallyAllVouchers)>0){
			$this->_objGetDBData->insertTallyVouchers($tallyAllVouchers);	
		}
