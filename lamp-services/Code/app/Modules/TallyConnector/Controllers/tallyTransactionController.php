...
class tallyTransactionController extends BaseController{
	...
	public function pushTallyLedgerCreditors($companyName){
		...
		foreach($supplierData as $suppler){
			...
			$GSTNType = $suppler->GSTIN_No=='' ? 'Unregistered' : 'Regular';
			...
			$postFields = array(
				'parentDC'			=>	'Sundry Creditors',
				'ledgerName'		=>	$ebutorSupplierName,
				'aliasName'			=>	$suppler->le_code,
				'openingBalance'	=>	'0',
				'address1'			=>	$address1,
				'address2'			=>	$address2,
				'city'				=>	$city,
			...
			if( $suppler->le_code!=''){
				...
				$response = $this->executeTallyAPI('createledgermaster', $postFields);
	...
	public function alterTallyLedgerCreditors(Request $request){
		...
		$supplierData = $this->_objGetDBData->getSupplierDataForAlter();
		...
		foreach($supplierData as $suppler){
			...
			$postDataArray = array(
				"suplierNameOld"			=> 	$ebutorSupplierName,
				"currencyName"				=>	"INR",
				"countryName"				=>	"India",
				"GSTNType"					=>	$GSTNType,
				"GSTINNo"					=>	$suppler->GSTIN_No,
				"stateName"					=>	$suppler->name,
				"companyName"				=>	$compnayFromInput
			);

			// if Supplier code is not entered then we are not making any entry
			if( $suppler->le_code!=''){

				$response = $this->executeTallyAPI('editLedgerMaster', $postDataArray);
				$response = json_decode($response, true);
				
				if(isset($response['status'])){
					
					if($response['status']=='Success'){
						// update table by 2 ( because it is altered )
						$ledgerData = $this->_objGetDBData->updateLedger($suppler->legal_entity_id, 2, $response['message']);
					}
				}	 
			}
		}
	...
	// Run the tally APIs
	public function executeTallyAPI($callerFunction, $postFields){
	...
	public function sendMailForLedger($emailList){
		...
		$subject='Tally Import Status For Ledger as on - ' . date('d-m-Y');
