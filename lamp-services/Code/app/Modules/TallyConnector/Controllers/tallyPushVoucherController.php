...
class tallyPushVoucherController extends BaseController{
	...
	public function pushTallyVouchers($companyName){
		...
		$activeCompanySyncURLs = $this->_objGetDBData->getActiveTallySyncURLs();		

		if(count($activeCompanySyncURLs)=='0') {
			return 'Please check tally_le_sync table';
		}


		foreach ($activeCompanySyncURLs as $companyInfo) {

		$costCentreGroup = $companyInfo->cost_centre_group;
		$costCentre = $companyInfo->cost_centre;

		// get Data from Supplier
		$voucherData = $this->_objGetDBData->getVoucherData($costCentreGroup,$costCentre);
      ...
      $ignoreList = ['Receipt','Payment'];
			if(!in_array($VCType, $ignoreList)) { //Adding receipt exception for multiple payments
        		$checkVoucherFlag = $this->_objGetDBData->checkDuplicateVoucher($VCNumber,$reffNumber, $VCType);
			}
			...
			if($checkVoucherFlag>0){
				...
				$voucherLineData = $this->_objGetDBData->getVoucherLineData($voucher->voucher_code, $voucher->voucher_type,$reffNumber);
			...
			if(isset($response['status'])){
				...
				if($response['status']=='Success'){
					// update table by 1
					$voucherData = $this->_objGetDBData->updateVoucher($voucher->voucher_code,$reffNumber, 1, $response['message']);
				}else{
					//update table by 0
                    $voucherData = $this->_objGetDBData->updateVoucher($voucher->voucher_code,$reffNumber, 0, $response['message']);   
				}
			}else{
				$voucherData = $this->_objGetDBData->updateVoucher($voucher->voucher_code,$reffNumber, 0, "No Response Received");
			}
		}
	...
	public function sendMailForVoucher($emailList){
		...
		$mailContent .= "<th>Cost Centre</th>";
		$mailContent .= "<th>VC CODE</th>";
		$mailContent .= "<th>VC Reference</th>";
		...
		foreach ($allPendingData as $pData) {
			...
			$mailContent .= "<tr>";
			$mailContent .= "<td>".$pData->cost_centre."</td>";
			$mailContent .= "<td>".$pData->voucher_code."</td>";
			$mailContent .= "<td>".$pData->reference_no."</td>";
		...
		$subject='Tally Import Status For Voucher as on - ' . date('d-m-Y');
		Log::info('vouchers to emails');
		Log::info($toEmails);
		//$mailContent="This is for testing";
		$body=array('template'=>'emails.tallyImportStatusMail','attachment'=>'','emailContent'=>$mailContent);
		Log::info($body);
