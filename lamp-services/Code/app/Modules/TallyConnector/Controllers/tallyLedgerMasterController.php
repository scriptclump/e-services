...
class tallyLedgerMasterController extends BaseController{
					...
					<DATA>
						<TALLYMESSAGE>
							<LEDGER NAME="'.trim($ledgerName).'" Action = "Alter">
								...
								<PARTYGSTIN>36AAAAB1234A1Z5</PARTYGSTIN>
    ...
    public function editLedgerMaster(Request $request){
        ...
        $compnayFromInput 		= trim($request->input('companyName'));
		$suplierNameOld 		= trim($request->input('suplierNameOld'));
		$currencyName 			= trim($request->input('currencyName'));
		$countryName 			= trim($request->input('countryName'));
		$GSTNType 				= trim($request->input('GSTNType'));
		$GSTINNo 				= trim($request->input('GSTINNo'));
		$stateName 				= trim($request->input('stateName'));
					...
					<DATA>
						<TALLYMESSAGE>
							...
							<LEDGER NAME="'.$suplierNameOld.'" Action = "Edit">
								...
								<GSTREGISTRATIONTYPE>'.$GSTNType.'</GSTREGISTRATIONTYPE>
