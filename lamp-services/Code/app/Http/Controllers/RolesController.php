...
class RolesController extends BaseController {
    ...
    public function index() {
        parent::Title('Roles - EBUTOR');
        $breadCrumbs = array('Dashboard' => url('/'), 'Administration' => '', 'Roles' => url('/roles/index'));
        parent::Breadcrumbs($breadCrumbs);
        $addRole = $this->roleAccessObj->checkPermissionByFeatureCode('RLE003');
        return View::make('roles.index')->with(['addRole' => $addRole]);
    }
    ...
    public function create() {
        try {
            ...
            return View::make('roles/add')->with(array('customers' => $customers, 'modules' => $modules, 'users' => $users, 
    ...
    public function insertRoleperMission() {
        try {
            $data = Input::get();
            $rolePermissions = $this->roleAccessObj->insertRoleperMission($data);
    ...
    public function edit($key_id) {
        try {
            ...
            $key_id = $this->roleAccessObj->decodeData($key_id);
            $breadCrumbs = array('Home' => url('/'), 'Administration' => '', 'Roles' => url('/roles/index'), 'Edit' => '#');
            parent::Breadcrumbs($breadCrumbs);
            parent::Title('Edit Role - EBUTOR');
            ...
            if ($roleId == 1) {
                $userinfo = db::table('user_roles')
                        ...
                        ->where(['user_roles.role_id' => $key_id])
                ...
                $userinfo = db::table('user_roles')
                        ->leftJoin('users', 'users.user_id', '=', 'user_roles.user_id')
                        ->select('users.mobile_no', 'user_roles.user_id', 'user_roles.role_id', 'users.firstname', 'users.lastname', 'users.email_id')
                        ->where(['user_roles.role_id' => $key_id, 'users.legal_entity_id' => $legalEntityId])
                        ->whereNotIn('user_roles.user_id', [$userId])
                        ->groupBy('users.user_id')
                        ->get()->all();
            }
            $selectedUsers = $this->roleAccessObj->secoundGridInUsers($key_id);
            $fullname = '';
            $profile_pic = '';
            $i = 0;
            ...
            $addPermission = $this->roleAccessObj->checkPermissionByFeatureCode('USR002');
            return View::make('roles/edit')->with(array('row' => $roles[0],
                        'addPermission' => $addPermission,
                        'modules' => $modules,
                        'userinfo' => $userinfo,
    ...
    public function delete($key_id) {
        try {
            $test = $this->roleAccessObj->DeleteRole($key_id);
